...
class TokenTextEncoder(TextEncoder):
  ...
  def _init_vocab(self, token_generator, add_reserved_tokens=True):
    ...
    self.store_encoder_decoder_to_file('~/')
    print('token_to_id.dict and id_to_token.dict saved to ~/')
    # print(self._token_to_id)

  def store_encoder_decoder_to_file(self, filename):
    with tf.gfile.Open(filename + 'token_to_id.dict', "w") as f:
      json.dump(self._token_to_id, f)
    with tf.gfile.Open(filename + 'id_to_token.dict', "w") as f:
      json.dump(self._id_to_token, f)
